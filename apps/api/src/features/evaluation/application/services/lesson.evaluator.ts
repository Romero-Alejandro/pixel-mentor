...
import { createLogger, type Logger as PinoLogger } from '@/shared/logger/logger.js';
...
} from '@/features/evaluation/application/dtos/evaluation.schemas';

import type { ILLMClient, LLMExecutionOptions } from '@/shared/llm/client.port';
import type {
  ISafePromptBuilder,
  PromptValues,
} from '@/features/prompt/domain/ports/safe-prompt-builder.interface';
import { SchemaValidationError } from '@/shared/errors/schema-validation.error';
import type { ISchemaValidator } from '@/features/evaluation/domain/ports/schema-validator.port';
...
export class LessonEvaluatorUseCase {
   ...
   * @param logger - Optional pino logger for error logging
   */
  constructor(
    private readonly llmClient: ILLMClient,
    promptBuilder: ISafePromptBuilder,
    private readonly schemaValidator: ISchemaValidator<unknown>,
    private readonly logger: PinoLogger = createLogger(undefined, {
