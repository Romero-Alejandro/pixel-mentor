...
export const envSchema = z
  ...
  .refine(
    (data) => {
      ...
      if (data.LLM_PROVIDER === 'groq' && !data.GROQ_API_KEY) return false;
      if (data.LLM_PROVIDER === 'openrouter' && !data.OPENROUTER_API_KEY) return false;
      return true;
    },
    {
      message: 'API key is required for the selected LLM_PROVIDER',
      path: ['LLM_PROVIDER'],
    },
  );
