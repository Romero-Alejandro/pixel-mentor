 ...
 * @example
 * ```typescript
 * import { featureFlagService, type CohortConfig } from '@/shared/config';
 *
...
// Create a logger for config loading warnings
const configLogger = createLogger(undefined, { name: 'config', level: 'warn' });
...
function loadFromEnvVar(): RawLoadedFlags | null {
  ...
  try {
    ...
    configLogger.warn('EVALUATION_FLAGS is not valid JSON, using defaults');
    return null;
  }
}
...
function loadFromConfigFile(configPath?: string): RawLoadedFlags | null {
  ...
  try {
    ...
    if (error instanceof SyntaxError) {
      configLogger.warn(`${filePath} is not valid JSON, using defaults`);
    } else {
      const errorMessage = error instanceof Error ? error.message : String(error);
      configLogger.warn(`Failed to read ${filePath}: ${errorMessage}`);
    }
    return null;
  }
}
 ...
 * @example
 * ```typescript
 * import { featureFlagService } from '@/shared/config';
 *
