...
import { cleanJsonResponse } from '@/shared/utils/ai-utils.js';
...
import { BaseGenerativeAdapter, BaseLLMAdapter } from '@/shared/ai/base-llm-adapter.js';
...
export class GeminiAIModelAdapter extends BaseGenerativeAdapter implements AIService {
  ...
  async generateAnswer(params: any): Promise<{ answer: string }> {
    ...
    try {
      ...
      this.logger?.error({ err: error }, '[Gemini] generateAnswer error');
