...
export function isEscalationRequired(parameters: {
  ...
  if (parameters.hasSafetyFlag) {
    return 'SAFETY_FLAG';
  }

  if (parameters.isOutOfScope) {
    return 'OUT_OF_SCOPE';
  }

  if (parameters.hasCriticalValidationError) {
    return 'CRITICAL_VALIDATION_ERROR';
  }

  if (parameters.failedAttempts > maxAttempts) {
