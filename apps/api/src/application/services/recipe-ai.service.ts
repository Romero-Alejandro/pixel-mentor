...
export class RecipeAIService {
  ...
  async generateRecipeDraft(input: GenerateRecipeDraftInput): Promise<GeneratedRecipeDraft> {
    ...
    console.log('[RecipeAI] Calling AI with prompt length:', prompt.length);
    ...
    try {
      ...
      console.log('[RecipeAI] AI response length:', response.answer.length);
      return this.parseAIResponse(response.answer, input);
    } catch (error) {
      console.error('[RecipeAI] AI generation error:', error);
...
## EJEMPLO "LAS VOCALES":
{
  "steps": [
    ...
    {"order":2,"stepType":"content","title":"La A","script":{"transition":{"text":"Conozamos la A"},"content":{"text":"La A es como una montaña. Su sonido es fuerte.","chunks":[{"text":"La A es como una montaña",500}]},"examples":[{"text":"MANZANA tiene A"}],"closure":{"text":"¡Muy bien!"}}},
  ...
  private getDefaultScript(stepType: unknown): GeneratedStep['script'] {
    ...
    if (stepType === 'question') {
      return {
        ...
        feedback: { correct: '¡Correcto!', incorrect: 'Pista: piensa en...' },
      };
    }
