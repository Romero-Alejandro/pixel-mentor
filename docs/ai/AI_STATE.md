...
- [x] [A1-ARCHITECT-FRONTEND-ALIGN] - Alineación frontend @apps/web/ con API @apps/api/ (Recipe-based)
- [x] [A2-LESSONPAGE-REDESIGN] - Rediseño de LessonPage (interfaz no-chat)
- [x] [BUGFIX-AUTH-TOKEN] - Fix: Token de autenticación se borraba tras dos recargas

### Fix: Token de autenticación se borraba tras dos recargas

**Causa raíz**: Race condition en el interceptor de axios - leía el token del estado de zustand antes de que se hidratara. Además, no había validación del token con el servidor al cargar la app.

**Solución aplicada**:

- **[apps/web/src/services/api.ts]** - Interceptor ahora lee token directamente de localStorage (no del store de zustand)
- **[apps/web/src/App.tsx]** - Agregado AuthInitializer que llama a checkAuth() al cargar la app para validar el token con el servidor

### Tareas de LessonPage Rediseñado

- [x] **[apps/web/src/components/question-hand/]** - Crear QuestionHandButton:
  - Botón siempre visible, bloqueado durante actividades
  - Cooldown de 30 segundos entre preguntas
  - Estados: idle, raised, listening, resolved

- [x] **[apps/web/src/components/activity-card/]** - Crear ActivityCard:
  - Opción múltiple (máx 5 opciones)
  - Feedback visual correcto/incorrecto
  - Control de intentos (max 3)

- [x] **[apps/web/src/components/content-panel/]** - Crear ContentPanel:
  - Muestra explanation/question/activity/completed según estado
  - Integración con ActivityCard

- [x] **[apps/web/src/pages/LessonPage.tsx]** - Reescribir con nueva estructura:
  - Layout: Mascot (izquierda) + Content Panel (derecha)
  - Botón "Tengo una duda" siempre visible
  - Controles de voz/escribir
  - Estados pedagógicos conectados a la UI

- [x] **[apps/web/src/components/mascot/Mascot.tsx]** - Actualizar estados del robot:
  - Agregar estados: waiting, hand_up, pointing_arms (Rive animations)

### Tareas de Mejora de Voz (TTS + Voice Input)

- [x] **[apps/web/src/components/voice-selector/]** - VoiceSelector:
  - 3 opciones de voz (Sofía, Mateo, Luna)
  - Configuración optimizada para niños
  - Guarda selección en localStorage

- [x] **[apps/web/src/hooks/useSpeech.ts]** - Enhanced useSpeech:
  - Voice settings configurables
  - Spanish voice filter
  - Confidence score

- [x] **[apps/web/src/components/voice-input/]** - VoiceInputWithConfirmation:
  - Flujo de confirmación con tutor
  - 5 frases variantes
  - Botones Sí/No
  - Fallback a texto si rechaza

### Tareas de Alineación Frontend → Backend

**Estado:** El frontend usa endpoints y tipos obsoletos (Lesson-based), mientras el backend ya migró a Recipe-based.

- [x] **[apps/web/src/services/api.ts]** - Actualizar endpoints:
  - `GET /api/lessons` → `GET /api/recipes`
  - `POST /api/leccion/start` → `POST /api/recipe/start` (body: `{ recipeId }`)
  - `POST /api/leccion/interact` → `POST /api/recipe/interact`

- [x] **[apps/web/src/services/api.ts]** - Agregar tipos faltantes:
  - `RecipeSchema` (importar de backend o recrear)
  - Actualizar `PedagogicalStateSchema`: agregar `CLARIFYING`, `COMPLETED`
  - `InteractRecipeResponseSchema`: agregar `feedback`, `isCorrect`, `extraExplanation`

- [x] **[apps/web/src/types/index.ts]** - Alineación de tipos:
  - Agregar tipo `Recipe`
  - `SessionCheckpoint.currentStepIndex` (reemplaza `currentSegmentIndex`)

- [x] **[apps/web/src/hooks/useLessonQueries.ts]** - Actualizar queries:
  - `useLessons()` → usar `api.listRecipes`
  - `useStartLesson()` → usar `api.startRecipe`
  - Agregar aliases para backwards compatibility

- [x] **[apps/web/src/pages/]** - Actualizar componentes:
  - `DashboardPage.tsx`: usar `listRecipes`, `recipeId`
  - `LessonPage.tsx`: actualizar `STATE_LABELS` con estados completos
  - `SessionPage.tsx`: usar `api.interactWithRecipe`

- [x] **[packages/shared/]** (CREADO) - Paquete de tipos compartidos:
  - `packages/shared/src/auth.ts` - UserSchema, AuthResponseSchema
  - `packages/shared/src/pedagogical.ts` - PedagogicalStateSchema, SessionCheckpointSchema
  - `packages/shared/src/recipe.ts` - RecipeSchema, StartRecipeOutputSchema, InteractRecipeOutputSchema
  - `packages/shared/src/session.ts` - SessionSchema, SessionStatusSchema
  - Frontend actualizado para importar desde `@pixel-mentor/shared`
  - Backend puede optar por usar shared o mantener DTOs propios con validaciones adicionales

### Validación Post-Alineación

- [x] `pnpm --filter @pixel-mentor/web typecheck` - Verificar tipos ✅
- [x] `pnpm --filter @pixel-mentor/api typecheck` - Verificar backend ✅
- [ ] `curl http://localhost:3001/health` - Health check API (requiere API corriendo)
- [ ] Testing manual de flujo: login → list recipes → start recipe → interact
...
- [x] **Renombrar clases UseCase de "Lesson" a "Recipe"**:
...
- [x] **Actualizar DTOs en `application/dto/index.ts`**:
...
- [x] **Eliminar casts `as any` en `OrchestrateRecipeUseCase`**:
...
- [x] **Actualizar controladores HTTP**:
...
- [x] **Actualizar barrel index de use-cases** (`application/use-cases/index.ts`):
...
- [x] **Actualizar tests de use-cases**:
...
- [x] **Migrar seeds y datos de prueba** (si existen) para usar `recipeId` en lugar de `lessonId`.
...
- [x] **Renombrar clases de use-cases de "Lesson" a "Recipe"**:
...
- [x] **Eliminar casts `as any` en `OrchestrateLessonUseCase`**:
...
- [x] **Actualizar controladores HTTP**:
  ...
  - Actualizar validación de DTOs en los controladores.
